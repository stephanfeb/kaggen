...
type WriteArgs struct {
	Path    string `json:"path" jsonschema:"required,description=The path to the file to write relative to the workspace."`
...
// newWriteTool creates a new write tool backed by the given VFS.
func newWriteTool(filesystem vfs.FS) tool.CallableTool {
	return function.NewFunctionTool(
		func(ctx context.Context, args WriteArgs) (*WriteResult, error) {
			return executeWrite(filesystem, args)
		},
...
func executeWrite(filesystem vfs.FS, args WriteArgs) (*WriteResult, error) {
	result := &WriteResult{
		Path: args.Path,
	}
	...
	// Write file
	if args.Append {
		f, err := filesystem.OpenFile(args.Path, appendFlags, 0644)
		if err != nil {
		...
		result.Bytes = n
		result.Message = fmt.Sprintf("Appended %d bytes to %s", n, args.Path)
		return result, nil
	}

	// Atomic write: write to temp file, then rename
	tmpFile := args.Path + ".tmp"
	if err := filesystem.WriteFile(tmpFile, []byte(args.Content), 0644); err != nil {
		result.Message = fmt.Sprintf("Error: failed to write temp file: %v", err)
		return result, fmt.Errorf("failed to write temp file: %w", err)
	}

	if err := filesystem.Rename(tmpFile, args.Path); err != nil {
		filesystem.Remove(tmpFile) // Clean up temp file on error
	...
	result.Message = fmt.Sprintf("Wrote %d bytes to %s", len(args.Content), args.Path)
	return result, nil
}

// appendFlags are the os file flags for append mode.
var appendFlags = os.O_APPEND | os.O_CREATE | os.O_WRONLY
